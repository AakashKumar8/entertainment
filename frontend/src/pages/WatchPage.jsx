...
const WatchPage = () => {
  ...
  const { contentType, bookmarks, addBookmark, removeBookmark } = useContentStore(); // Use store
  ...
  // Handle next and previous trailer
  const handleNext = () => {
  ...
  // Handle scrolling for similar content
  const scrollLeft = () => {
  ...
  const handleBookmark = () => {
    if (!bookmarks.some((bookmark) => bookmark.id === content?.id)) {
      addBookmark(content); // Add content to bookmarks
    } else {
      removeBookmark(content?.id); // Remove content from bookmarks
    }
  };
      ...
      <div className="mx-auto container px-4 py-8 h-full">
        ...
        <div className="flex justify-center mt-6">
          ...
          >
            ...
            {bookmarks.some((bookmark) => bookmark.id === content?.id)
              ? "Remove Bookmark"
              : "Add to Bookmarks"}
          </button>
        </div>
